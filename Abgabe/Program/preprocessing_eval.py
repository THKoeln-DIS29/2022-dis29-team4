...
def pre_eval():
    ...
    print("starting preprocessing_eval script...")
    ...
    print("The Files are located at : " + str(directory))
    for n in tqdm(range(5)):
        tar = tarfile.open(directory+'/testdata1_'+str(n+1)+'.tar.gz')
        for member in tqdm(tar.getmembers()):
            f = tar.extractfile(member).read()
            with gzip.GzipFile(fileobj=BytesIO(f)) as fp:
                df = pd.read_csv(fp, usecols=['actor_account_id','logid','log_detail_code','entity_code','actor_level','actor_job','time','actor_faction','actor_faction2','target_account_id','session','actor_id','new_value4_num','old_value2_num','new_value3_num','use_value2_num'],low_memory=False)
                dict_merge = {}
                ...
                try:
                    ...
                    dict_merge['duels_per_session'] = 0
                try:
                    dict_merge['partybattle_num'] = len(df[df.entity_code==80])
                except ZeroDivisionError:
                    dict_merge['partybattle_num'] = 0
                try:
                ...
                try:
                    ...
                    dict_merge['completechallengetoday_num'] = 0
                try:
                    dict_merge['completechallengeweek_num'] = len(df[df.logid==5015])
                except ZeroDivisionError:
                    dict_merge['completechallengeweek_num'] = 0
                try:
                ...
                try:
                    ...
                    dict_merge['average_time_between_logins'] = 0
                ...
                try:
                    ...
                    dict_merge['money_max'] = 0
                try:
                    dict_merge['gathering_num'] = len(df[df.logid==2405])
                except ValueError:
                    dict_merge['gathering_num'] = 0
    ...
    # %%
    features.to_csv('output/eval.csv', sep=',', float_format='{:.2f}'.format)
    print("saved data to: output/features.csv")
    # %%
